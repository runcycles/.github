...
# and synced to all Java repos via scripts/sync-claude-config.sh.
#
...
#
# Issue: runcycles/.github#62
# Safety: settings.xml writes are guarded — if the file already exists, the
# script logs a warning and skips the overwrite. This avoids clobbering
# pre-existing Maven config (mirrors, Sonatype Central creds, alternate
# proxies) when running in environments where the user has set up their own
# settings.xml. The mvn-proxy wrapper install is independent and always runs.
...
# Create Maven settings.xml with proxy config — but only if no settings.xml
# already exists. Overwriting silently destroys user mirror/auth config.
mkdir -p ~/.m2
if [ -e "$HOME/.m2/settings.xml" ]; then
  echo "session-start-maven-proxy: ~/.m2/settings.xml exists; skipping proxy write." >&2
  echo "  To regenerate, remove the file or merge proxy entries manually." >&2
  echo "  Reference proxy host: ${PROXY_HOST}:${PROXY_PORT}" >&2
else
